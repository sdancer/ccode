...
// Lines: 149783-149890
//
var createRenderState = U((o8G, gBB)=>{
    gBB.exports = function() {
...
function LG(A) {
    ...
    if (Q) {
        let G = 0;
        for(let Z = 0; Z < A.length; Z++)if (A.charCodeAt(Z) > 31) G++;
        return G;
    }
    if (A.includes("\x1B")) {
        if (((A = yX(A)), A.length === 0)) return 0;
    }
    if (!HE8(A)) {
        let G = 0;
        for (let Z of A){
    ...
    let B = 0;
    for (let { segment: G } of VE8.segment(A)){
        if (((mBB.lastIndex = 0), mBB.test(G))) {
            B += DE8(G);
            continue;
        }
        for (let Z of G){
